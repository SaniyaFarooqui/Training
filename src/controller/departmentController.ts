...
class DepartmentController {
    ...
    public CreateDepartment = async (req: Request, res: Response) => {
        let DepartmentData = req.body as departmentType;
        if (DepartmentData == null || DepartmentData == undefined) {
            res.status(400).json({ error: "please provide the data" });
        } else {
            try {
                let response = await this.department_service.CreateDepartment(DepartmentData as any);
                if (response == null || response == undefined) {
                    res.status(400).json({ error: "could not create department, please try again" });
                } else {
                    res.status(200).json({ message: "Department created successfully", data: response });
                }
            } catch (error: any) {
                if (error.errors) {
                    let validationError = [];
    ...
    public UpdateDepartment = async (req: Request, res: Response) => {
        ...
        if (id == null || id == undefined || id === ":id") {
            ...
            try {
                let response = await this.department_service.UpdateDepartment(id, DepartmentData as any);
                if (response == null || response == undefined) {
